(function exposeScreenCamPreview(root, factory) {
  ...
  function defaultVideoCapabilities() {
    if (typeof RTCRtpReceiver === 'undefined'
        || typeof RTCRtpReceiver.getCapabilities !== 'function') return null;
    return RTCRtpReceiver.getCapabilities('video');
  }

  function fmtpParameter(codec, name) {
    if (!codec || typeof codec.sdpFmtpLine !== 'string') return null;
    const expected = String(name).toLowerCase();
    for (const rawPart of codec.sdpFmtpLine.split(';')) {
      const separator = rawPart.indexOf('=');
      if (separator < 0) continue;
      const key = rawPart.slice(0, separator).trim().toLowerCase();
      if (key === expected) return rawPart.slice(separator + 1).trim().toLowerCase();
    }
    return null;
  }

  function isH264MainPacketizationMode1(codec) {
    return String(codec?.mimeType || '').toLowerCase() === 'video/h264'
      && fmtpParameter(codec, 'packetization-mode') === '1'
      && /^4d[0-9a-f]{4}$/.test(fmtpParameter(codec, 'profile-level-id') || '');
  }

  // El encoder NVENC de ScreenCam publica H.264 Main. Chrome suele ofrecer
  // Constrained Baseline antes que Main y MediaMTX elige el primer perfil
  // compatible, aunque luego retransmite el bitstream Main sin transcodificar.
  // Solo reordenamos capacidades que el navegador ya anuncio y conservamos
  // todos los fallbacks para no reducir compatibilidad.
  function preferH264MainCodec(transceiver, capabilities) {
    ...
    const preferred = codecs.filter(isH264MainPacketizationMode1);
    if (preferred.length === 0) return false;
    const fallback = codecs.filter((codec) => !isH264MainPacketizationMode1(codec));
    try {
      transceiver.setCodecPreferences([...preferred, ...fallback]);
      return true;
    } catch (_) {
      // Navegadores antiguos pueden exponer capacidades pero no aceptar la
      // preferencia. La negociacion por defecto sigue siendo el fallback.
      return false;
    }
  }
  ...
  function createWhepController(config) {
    const {
      ...
      getVideoCapabilities = defaultVideoCapabilities,
    ...
    async function runAttempt() {
      ...
      try {
        ...
        // La deteccion es opcional; la oferta por defecto sigue disponible.
      }
      preferH264MainCodec(videoTransceiver, videoCapabilities);
      pc.ontrack = (ev) => {
